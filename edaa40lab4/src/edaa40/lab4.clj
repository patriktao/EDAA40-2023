(ns edaa40.lab4
  (:require [edaa40.core :refer [square?]]))
...
;;     iff a and b are in A and their sum is a square number"
  ...
  (set (for [a A b A :when (square? (+ a b))] [a b]))
...
;;      If no such path exists, it should return nil."
  ...
  {:pre [(not (contains? S a))
         (not (contains? (set P) a))
         (empty? (intersection S (set P)))]
   :post [(or (empty? %) (= (set %) (union S (set P) #{a})))
          (or (empty? %) (= (count %) (+ (count S) (count P) 1)))]}

  (if (empty? S) (concat P [a]) (some #(H' E % (disj S %) (concat P [%]))  ;;testar tills den första true
                                      (intersection S (image-of E a))))
...
;; 
  ...
  (intersection (set (map #(add-move pos %) Moves)) ;; ;; The set of all moves from pos
                B) ;; The board
...
(defn create-knights-move-relation
  ...
  [B]
  (set
   (for [pos B  ;;for each valid position pos on the board
         next-pos (next-positions pos B) ;; for each position next-pos that can be reached from pos
         ]
     [pos next-pos] ;; returns a relation with the pair (pos next-pos)
     ))
