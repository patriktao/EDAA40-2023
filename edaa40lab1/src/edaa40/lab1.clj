...
(defn image-of
  ...
  ;; ta fram varje tuppel r i Relation R där första elementet = x
...
(defn reflexive?
  ...
  ;; kollar om varje element mappas till sig själv

  (every? #(contains? R [% %]) A))
;; every checkar om alla elementet i vektorn uppnår conditionen
...
;;alla elementen är symmetriska, asså att (a,b) = (b,a)
...
(defn transitive?
  ...
  ;;om det första elementet är kopplat till det andra elementet och den andra komponenten är associerat med det tredje elementet, måste det första elementet korreleras med det tredje elementet.
...
(defn injective?
  ...
  ;; one-to-one relation som mappar elementen av domainen till element i codomainen, disctinctively. 
  ;; varje nod har en pil
  ;; f(x1) = f(x2) vilket innebär att x1=x2
  [f]
  ;;
  (let [g (inverse f)]
    (every?
...
(defn surjective?
  ...
  (= (rng f) B))
   ;; uses "rng"
   ;; du har en funktion f, och alla element y ska kunnas mappas från element x så att f(x) = y
   ;; domain -> codomain
...
(defn bijective?
  ...
  [f B]
 ;; innebär att alla element kan mappas från domain till codomain, distinctively
